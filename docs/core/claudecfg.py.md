...
`default_permission_mode()` читает `permissions.defaultMode` из цепочки настроек claude (пользовательские, проектные, локальные — последние сильнее). Это позволяет сказать, в каком режиме запустится сессия, не запуская её: транспорт показывает режим в приветствии до того, как появится первый `system/init`.
