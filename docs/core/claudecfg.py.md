...
`known_projects()` — реальные пути из `projects` общего конфига (ключи — каталоги, в которых claude
запускали), существующие на диске, без дублей по нормализованному пути, новые первыми. Это то, из
чего Telegram-демон строит `/projects`.
