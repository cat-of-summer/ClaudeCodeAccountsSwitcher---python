...
ccas menu                       # выбрать активный аккаунт интерактивно
...
Момент перехода определяется не опросом API, а фактом: claude записывает в транскрипт сессии строку с `"error":"rate_limit"`, когда действительно упёрся в стену. До этого он может часами гонять длинный инструмент — прерывать его нельзя, и ccas не прерывает.

Дальше ccas сверяется с API (временный 429 в транскрипте выглядит так же, как исчерпанная квота), выбирает наименее нагруженный аккаунт со свободными лимитами и перезапускает сессию на нём — с тем же id разговора и в том же режиме, включая plan. Аккаунта со свободными лимитами нет — перехода не будет, сессия останется как есть.

Несколько открытых терминалов на одном аккаунте упираются в стену почти одновременно. Чтобы они не разъехались по разным аккаунтам, первый записывает `switch-plan.json`, а остальные ему следуют.
...
| `auto-switch-prompt` | пусто | что отправить в восстановленную сессию сразу после перехода |
...
|---|---|
| `test_arguments` | разбор `2`, `@work`, `--`, подкоманды, голый промпт; подмешивание флагов |
...
| `test_usage` | разбор `resets_at`, дата+время отката у каждого из двух окон, откат формата при отсутствующем ключе каталога, вывод процентов, устойчивость к битым данным, три яруса TTL, обе формы ответа API, обмен токена перед запросом |
...
| `test_autoswitch` | когда сессия супервизируется, сборка команды перезапуска, вотчер транскрипта, выбор цели, договор терминалов |
