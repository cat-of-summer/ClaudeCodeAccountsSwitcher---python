...
`ccas list` показывает сохранённые лимиты и молча обновляет по сети те, что старше десяти минут. `--refresh` опрашивает все принудительно, `--cached` не ходит в сеть вовсе.
...
| `test_usage` | разбор `resets_at`, дата+время отката у каждого из двух окон, откат формата при отсутствующем ключе каталога, вывод процентов, устойчивость к битым данным, TTL кэша лимитов, обе формы ответа API |
